...
function sanitizeRecipeJson(data, fallbackDish) {
  ...
  const optional = Array.isArray(data.optional)
    ? data.optional.map((x) => String(x || '').trim()).filter(Boolean).slice(0, 8)
    : [];

  const steps = Array.isArray(data.steps)
    ? data.steps.map((x) => String(x || '').trim()).filter(Boolean).slice(0, 7)
    : [];
...
- 영상에 없는 계량을 보완할 수는 있지만, 그 경우 "영상 그대로의 레시피"라고 주장하지 않는다.
- 특정 영상 제작자나 유명인이 그 쿠팡 상품을 사용했다고 지어내지 않는다.
- secretIngredient는 소스, 양념, 육수, 장류, 오일, 향신료처럼 쿠팡에서 상품 검색하기 좋은 식재료를 우선한다.
- 효능/건강효과를 주장하지 않는다.
- 음슴체 금지.
- 계량은 한국 사용자가 이해하기 쉽게 "1스푼, 1/2스푼, 100ml, 1팩" 등으로 쓴다.
...
위 정보를 참고해서 실제 따라 하기 좋은 레시피 JSON을 만들어줘.`;
...
function buildRecipeCommentText(recipe) {
  ...
  return `🌿 ${recipe.dishName} (${recipe.servings})\n\n${ingredientLines}${optionalLine}\n\n✔ 만드는 순서\n${steps}\n\n내 비밀 소스는 ${recipe.secretIngredient}ㅋㅋ${reason}\n이 재료는 아래 링크에서 확인하면 돼👇`;
}
...
async function chooseCoupangProduct(accountId, searchKeyword) {
  ...
  const preferred = products.find((p) => p.isRocket) || products[0];
  return preferred;
}
