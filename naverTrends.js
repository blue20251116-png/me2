...
async function rankKeywordsByTrend(accountId, keywords) {
  ...
  if (!account?.naver_client_id || !account?.naver_client_secret) {
    return null; // 네이버 API 키 미설정 - 트렌드 비교 없이 넘어가라는 뜻
  }
  ...
  const res = await axios.post(
    ...
    {
      headers: {
        'X-Naver-Client-Id': account.naver_client_id,
        'X-Naver-Client-Secret': account.naver_client_secret,
