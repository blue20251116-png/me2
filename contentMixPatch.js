...
fs.readFileSync=function(filename,...args){
  ...
  if(!applied&&path.resolve(String(filename))===target){
    ...
    console.log('[Autopilot][CONTENT MIX PATCH] 45/45/5/5 + persistent cursor + lifestyle-slot-lock + special-story + product-lock + publish-rate-first 완료');
    return isBuffer?Buffer.from(transformed,'utf8'):transformed;
  }
  return data;
};

console.log('[Autopilot][CONTENT MIX PATCH] 일반45/레시피45/일상5/특수상품썰5 · 재배포해도 순번 유지 · lifestyle 슬롯 실제 강제 · 후보3개/exact identity 유지');
