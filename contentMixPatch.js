...
fs.readFileSync=function(filename,...args){
  ...
  if(!applied&&path.resolve(String(filename))===target){
    ...
    console.log('[Autopilot][CONTENT MIX PATCH] 45/45/10 + lifestyle text-only + newline normalization 완료');
    return isBuffer?Buffer.from(transformed,'utf8'):transformed;
  }
  return data;
};

console.log('[Autopilot][CONTENT MIX PATCH] 일반45/레시피45/일상10 · lifestyle 사진/영상 없음');
