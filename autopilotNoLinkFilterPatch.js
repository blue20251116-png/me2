...
require.extensions['.js'] = function patchedJsLoader(mod, filename) {
  ...
  // OpenAI가 Threads/Instagram CDN을 직접 다운로드하면 403이 날 수 있으므로
  // Railway 서버가 먼저 이미지를 받고 data URL로 바꿔 Vision 요청에 넣는다.
  source = source.replace(
  ...
  // 상품명은 동일 카테고리 동의어를 허용하되 완전히 다른 상품군은 계속 차단한다.
  source = source.replace(
  ...
  // 쿠팡 검색 401이 어느 계정/검색어에서 발생했는지 즉시 알 수 있게 한다.
  source = source.replace(
  ...
  // sold를 1차 검색어로 고정하고 동일 상품군 안에서만 2차 검색어를 허용한다.
  source = source.replace(
