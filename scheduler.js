...
async function buildCommentText(account, post) {
  ...
  try {
    ...
    console.error(`[댓글 연결문구 생성 실패, 고지문만 사용] account #${account.id} post #${post.id}:`, err.message);
  }
...
async function postAffiliateComment(account, post, parentMediaId) {
  ...
  try {
    ...
    db.prepare(
      `UPDATE posts
       SET comment_status = 'failed', comment_error_message = ?
       WHERE id = ?`
    ).run(msg, post.id);
...
function chooseContentMode() {
  return Math.random() < 0.35 ? 'story' : 'product';
}
...
async function runAutopilotOnce(account) {
  ...
  const contentMode = chooseContentMode();
  ...
  db.prepare(
    `UPDATE accounts
     ...
     WHERE id = ?`
  ).run(keyword, target, account.id);

  console.log(
    `[자동발행 예약] account #${account.id} mode="${contentMode}" target="${target}" keyword="${keyword}" (${trendNote}) product="${picked.name}" image="${imageUrl ? '원본 상품컷 1장' : '없음'}"`
  );
}
...
function startAutopilotJob() {
  cron.schedule('* * * * *', async () => {
    ...
    for (const account of dueAccounts) {
      ...
      db.prepare(
        `UPDATE accounts SET autopilot_next_at = ? WHERE id = ?`
      ).run(nextAt, account.id);
