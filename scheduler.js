...
function buildDisclosureText(account, link) {
  return (account.coupang_disclosure_template || '{link}').replace('{link}', link);
}
...
function combineCommentSafely(prefixText, disclosure, maxLength = 480) {
  ...
  const sep = prefixText && d ? '\n\n' : '';
  if (!prefixText) return trimToLimit(d, maxLength);
  if (!d) return trimToLimit(prefixText, maxLength);
  const available = maxLength - d.length - sep.length;
  if (available <= 0) return trimToLimit(d, maxLength);
  return `${trimToLimit(prefixText, available)}${sep}${d}`;
}
    ...
    for (const accountSummary of listAllAccountsForSystem()) {
      ...
      for (const post of duePosts) {
        ...
        try {
          ...
          db.prepare(`UPDATE posts SET status='posted', threads_media_id=?, posted_at=? WHERE id=?`).run(
            mediaId,
            new Date().toISOString(),
            post.id
          );
          db.prepare(
            `INSERT INTO insights (post_id,views,likes,replies,reposts,quotes) VALUES (?,0,0,0,0,0) ON CONFLICT(post_id) DO NOTHING`
          ).run(post.id);
          ...
          const msg = apiErr
            ? `${apiErr.message} (type: ${apiErr.type || '-'}, code: ${apiErr.code || '-'}${apiErr.error_subcode ? ', subcode: ' + apiErr.error_subcode : ''})`
            : err.message;
...
function chooseContentMode() {
  const r = Math.random();
  if (r < 0.70) return 'recipe';
  if (r < 0.90) return 'story';
  return 'product';
}

function saveAutopilotPost({ accountId, text, link, imageUrl, extraImageUrl, recipeCommentText = null }) {
  db.prepare(
    `INSERT INTO posts (text,link,image_url,extra_image_url,scheduled_at,auto_comment_enabled,comment_status,account_id,recipe_comment_text)
     VALUES (?,?,?,?,?,1,'pending',?,?)`
  ).run(text, link, imageUrl, extraImageUrl, new Date().toISOString(), accountId, recipeCommentText);
}
...
async function runAutopilotOnce(account) {
  ...
  if (contentMode === 'story') {
    ...
    const variants = await generateCaption(account.id, {
      productName: picked.name,
      price: picked.price,
      youtubeSource,
    });
    text = Array.isArray(variants)
    ...
    if (youtubeSource?.thumbnail) {
      ...
      imageSourceLabel = extraImageUrl ? 'YouTube 썸네일 + 상품컷' : 'YouTube 썸네일';
    } else if (picked.image) {
      imageUrl = picked.image;
      imageSourceLabel = '원본 상품컷 1장';
    }

    if (account.autopilot_frame_media_enabled && !youtubeSource?.thumbnail) {
      try {
        ...
        console.log(`[Media] 저장 프레임 조회 실패 — 상품 이미지 유지: ${err.message}`);
      }
    }
  }
...
function startAutopilotJob() {
  ...
  cron.schedule('* * * * *', async () => {
    ...
    for (const summary of listAllAccountsForSystem()) {
      ...
      if (!account.autopilot_enabled) {
        nextRunAt.delete(account.id);
        continue;
      }
