...
function buildDoubleLinkComment(account,prefix,link,maxLength=490){
  const cap=Math.min(490,Math.max(1,Number(maxLength)||490));
  const p=String(prefix||'').trim();
  const l=String(link||'').trim();
  let disclosure=isCoupangLink(l)?buildDisclosureOnly(account):'';
  const links=l?`${l}\n${l}`:'';
  let tail=[links,disclosure].filter(Boolean).join('\n\n');
  if(tail.length>cap&&isCoupangLink(l)){
    disclosure=DEFAULT_COUPANG_DISCLOSURE;
    tail=[links,disclosure].filter(Boolean).join('\n\n');
  }
  if(tail.length>cap)throw new Error(`댓글 필수영역(링크 2개+고지문)이 Threads ${cap}자 제한을 초과했습니다: ${tail.length}자`);
  if(!p)return tail;
  const available=cap-tail.length-2;
  if(available<=0)return tail;
  const head=trimToLimit(p,available);
  const out=head?`${head}\n\n${tail}`:tail;
  if(out.length>cap)throw new Error(`댓글 길이 조립 오류: ${out.length}/${cap}자`);
  return out;
}
...
async function buildCommentText(account,post){
  if(hasCoupangKeys(account)&&post.recipe_comment_text&&!post.link)throw new Error('쿠팡 자동댓글 링크가 비어 있어 댓글 발행을 중단했습니다');
  if(!post.link)return trimToLimit(String(post.recipe_comment_text||'').trim(),490);
  return buildDoubleLinkComment(account,post.recipe_comment_text||'',post.link,490);
}

function nextCommentRetryIso(retryCount){
  const delayMinutes=Math.min(10,Math.max(1,retryCount)*2);
  return new Date(Date.now()+delayMinutes*60*1000).toISOString();
}

async function postAffiliateComment(account,post,parentMediaId,{isRetry=false}={}){
  if(!post.auto_comment_enabled)return false;
  if(!post.link&&!post.recipe_comment_text)return false;
  try{
    const commentText=await buildCommentText(account,post);
    if(!commentText)return false;
    if(commentText.length>490)throw new Error(`댓글 최종 길이 검증 실패: ${commentText.length}자`);
    console.log(`[댓글 길이] account #${account.id} post #${post.id}: ${commentText.length}자/490 link=${post.link?'yes':'no'} retry=${isRetry?'yes':'no'}`);
    const commentMediaId=await publishReply(account.id,parentMediaId,commentText);
    db.prepare(`UPDATE posts SET comment_status='posted', comment_media_id=?, comment_posted_at=?, comment_error_message=NULL, comment_retry_count=0, comment_next_retry_at=NULL WHERE id=?`).run(commentMediaId,new Date().toISOString(),post.id);
    console.log(`[댓글 등록 완료] account #${account.id} post #${post.id} -> comment ${commentMediaId}`);
    return true;
  }catch(err){
    const msg=err.response?.data?.error?.message||err.message;
    const retryCount=Number(post.comment_retry_count||0)+1;
    const nextRetry=retryCount<3?nextCommentRetryIso(retryCount):null;
    db.prepare(`UPDATE posts SET comment_status='failed', comment_error_message=?, comment_retry_count=?, comment_next_retry_at=? WHERE id=?`).run(msg,retryCount,nextRetry,post.id);
    console.error(`[댓글 등록 실패] account #${account.id} post #${post.id}: ${msg} retry=${retryCount}/3 next=${nextRetry||'stop'}`);
    return false;
  }
}

async function retryFailedComments(account,now){
  const failed=db.prepare(`SELECT * FROM posts WHERE account_id=? AND status='posted' AND auto_comment_enabled=1 AND comment_status='failed' AND comment_media_id IS NULL AND threads_media_id IS NOT NULL AND COALESCE(comment_retry_count,0)<3 AND (comment_next_retry_at IS NULL OR comment_next_retry_at<=?) ORDER BY posted_at ASC LIMIT 3`).all(account.id,now);
  for(const post of failed){
    console.log(`[댓글 재시도] account #${account.id} post #${post.id} retry=${Number(post.comment_retry_count||0)+1}/3`);
    await postAffiliateComment(account,post,post.threads_media_id,{isRetry:true});
...
function startPublishJob(){
  cron.schedule('* * * * *',async()=>{
    ...
    for(const s of listAllAccountsForSystem()){
      ...
      await retryFailedComments(account,now);
      const due=db.prepare(`SELECT * FROM posts WHERE account_id=? AND status='pending' AND scheduled_at<=? ORDER BY scheduled_at ASC`).all(account.id,now);
      for(const post of due){
        ...
        try{
          let mediaId;
          if(post.video_url){
            mediaId=await publishPost(account.id,{text:post.text,imageUrl:null,videoUrl:post.video_url});
          }else if(post.image_url&&post.extra_image_url){
            mediaId=await publishCarouselPost(account.id,{text:post.text,imageUrls:[post.image_url,post.extra_image_url]});
          }else{
            mediaId=await publishPost(account.id,{text:post.text,imageUrl:post.image_url,videoUrl:null});
          }
          if(!mediaId)throw new Error('Threads 발행 결과 mediaId가 없습니다');
          const postedAt=new Date().toISOString();
          db.prepare(`UPDATE posts SET status='posted', threads_media_id=?, posted_at=?, error_message=NULL WHERE id=?`).run(mediaId,postedAt,post.id);
          if(account.user_id){logUsage(account.user_id,'publish');console.log(`[발행 횟수 차감] user #${account.user_id} post #${post.id} - Threads 발행 성공 확인 후 1회 차감`);}
          ...
          const freshPost=db.prepare(`SELECT * FROM posts WHERE id=?`).get(post.id)||post;
          await postAffiliateComment(account,freshPost,mediaId);
        }catch(err){
          ...
          console.error(`[발행 실패][횟수 차감 없음] account #${account.id} post #${post.id}:`,msg);
        }
      }
    }
  });
...
function saveAutopilotPost({accountId,text,link,imageUrl,extraImageUrl,videoUrl=null,recipeCommentText=null}){
  db.prepare(`INSERT INTO posts (text,link,image_url,extra_image_url,video_url,scheduled_at,auto_comment_enabled,comment_status,account_id,recipe_comment_text,comment_retry_count,comment_next_retry_at) VALUES (?,?,?,?,?,?,1,'pending',?,?,0,NULL)`).run(text,link||null,imageUrl||null,extraImageUrl||null,videoUrl||null,new Date().toISOString(),accountId,recipeCommentText);
}
...
function chooseSourceMedia(result){
  ...
  if(videos.length>=1)return{videoUrl:videos[0],imageUrl:null,extraImageUrl:null,imageSourceLabel:'Threads 소재 원본 영상 1개'};
  if(images.length>=2)return{videoUrl:null,imageUrl:images[0],extraImageUrl:images[1],imageSourceLabel:'Threads 소재 원본 이미지 2장'};
  if(images.length===1)return{videoUrl:null,imageUrl:images[0],extraImageUrl:null,imageSourceLabel:'Threads 소재 원본 이미지 1장'};
  return{videoUrl:null,imageUrl:result?.product?.image||null,extraImageUrl:null,imageSourceLabel:result?.product?.image?'Threads 미디어 없음 → 쿠팡 상품 이미지 1장':'미디어 없음'};
}
...
async function runAutopilotOnce(account){
  ...
  if(!hasCoupangKeys(account)){await runContentOnlyAutopilot(account,target);return;}
  ...
  const media=chooseSourceMedia(result);
  ...
  console.log(`[자동발행 예약][V10 VIDEO-FIRST-REPLY-RETRY] account #${account.id} target="${target}" mode="${result.mode}" topic="${result.topic}" product="${result.product.name}" source="${result.sourceUrl}" media="${media.imageSourceLabel}" affiliateLink=yes`);
}
