...
function splitThreadsSentences(text){let t=String(text||'').replace(/\r/g,' ').replace(/\n+/g,' ').replace(/\s+/g,' ').trim();if(!t)return[];const chunks=t.split(/(?<=[.!?])\s+|(?<=ㅋㅋ)\s+|(?<=ㅎㅎ)\s+/).map(x=>x.trim()).filter(Boolean);const out=[];for(const chunk of chunks){if(chunk.length<=54){out.push(chunk);continue;}let rest=chunk;while(rest.length>54){let cut=Math.max(rest.lastIndexOf(' ',54),rest.lastIndexOf(',',54));if(cut<24)cut=54;out.push(rest.slice(0,cut).trim());rest=rest.slice(cut).trim();}if(rest)out.push(rest);}return out.filter(Boolean);}
function formatThreadsBody(text){const sentences=splitThreadsSentences(text).slice(0,8);if(!sentences.length)return'';const paragraphs=[];let i=0;while(i<sentences.length&&paragraphs.length<5){const remaining=sentences.length-i;const remainingSlots=5-paragraphs.length;let take=1;if(remaining>remainingSlots&&paragraphs.length>0)take=2;if(paragraphs.length===0&&sentences[i].length<30&&remaining>=2&&sentences[i+1].length<32)take=2;const block=sentences.slice(i,i+take).join('\n').trim();if(block)paragraphs.push(block);i+=take;}if(i<sentences.length&&paragraphs.length){const extra=sentences.slice(i).join(' ').trim();if(extra){const last=paragraphs.pop();paragraphs.push(`${last}\n${extra}`);}}return paragraphs.filter(Boolean).slice(0,5).join('\n\n').trim();}
