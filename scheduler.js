...
function compactRecipePrefix(prefix,limit){
  ...
  if(!cap)return'';
  const raw=String(prefix||'').replace(/\r/g,'').trim();
  if(!raw)return'';
  ...
  if(!isRecipe){
    ...
    for(const line of lines){const add=(out.length?1:0)+line.length;if(used+add>cap)break;out.push(line);used+=add;}
    return out.join('\n');
  }
  const ingredientHeader='🥘 재료';
  const methodHeader='🍳 만드는 법';
  let ingredients=[];let methods=[];
  ...
  let ing=ingredients.slice(0,8),steps=methods.slice(0,4),out=render(ing,steps);
  while(out.length>cap&&steps.length>1){steps.pop();out=render(ing,steps);}
  while(out.length>cap&&ing.length>3){ing.pop();out=render(ing,steps);}
  if(out.length<=cap&&ing.length&&steps.length)return out;
  // 최소한 재료와 만드는 법 둘 다 완결된 문장으로 남긴다. 문장 중간 절단은 하지 않는다.
  ...
  // 원본 구조가 불완전하면 완전한 줄만 남긴다.
  const safe=[];let used=0;
  for(const line of lines){const add=(safe.length?1:0)+line.length;if(used+add>cap)break;safe.push(line);used+=add;}
  return safe.join('\n');
}
function buildDoubleLinkComment(account,prefix,link,maxLength=490){
  ...
  const available=cap-tail.length-(tail?2:0);
