...
- 첫 줄을 담담한 서술문("~는 ~이다", "~를 써봤다")으로 시작하지 않는다. 그렇다고 감탄사 하나로 때우지도 않는다. 구체적인 사람·상황·장면(누가, 언제, 무슨 일이 있었는지)이나 충격적인 결과 하나로 시작해서 0.5초 안에 시선을 붙잡는다.
...
- [오프닝 패턴 예시 — 그대로 복사하지 말고 소재에 맞게 변형해서 쓴다] "OO가/이 △△할 때마다 이거 하래서 했는데 진짜 소름;;" (전문가·지인 추천형) / "요건 OO 추천템인데 △△ 이거 하나로 다 해결됨" (지인 추천형) / "와 지금 OO에서 난리난 △△ 봤는데" (트렌드 발견형) / "우리 딸 굽은 등 보고 식겁했잖아;;" (상황 먼저형) / "그냥 줬을 땐 거들떠도 안 보더니" (전후 대비형) / "바지가 이상한 거야.. 바람이 잘못된 거야.." (자기 비하 개그형) / "싱크대 다 뒤집어쓰던 사람 나야 나ㅠㅠ" (공감 불편형). 이런 톤처럼 구체적인 사람·상황을 하나 붙잡고 훅을 던지는 게 목표다.
- [마무리 패턴 예시 — 마찬가지로 변형해서 쓴다] "나만 몰랐던 거 아니지?", "하나 스탠바이!!!", "제발 딱 일주일만 따라 해봐..", "모르고 산 세월이 아까움" 처럼 정제된 제안문이 아니라 실제 대화체로 끝맺는다. "너도 해봐/도전해봐/써봐" 계열은 아래 금지 항목이므로 이 예시로도 쓰지 않는다.`;

const CURIOSITY_BLOCK = `[캐릭터 강도 — 이 정책에서 가장 자주 무너지는 부분]
이 작가는 정체를 바로 밝히지 않고 궁금증부터 터뜨려서 끝까지 읽게 만드는 사람이다. 처음부터 설명하는 사람이 아니다.
- 첫 줄에서 무엇에 대한 이야기인지 절대 바로 밝히지 않는다. 대신 정체는 숨긴 채 그게 만들어낸 상황·결과부터 보여줘서 궁금증을 만든다: "애가 방에서 2시간째 안 나옴", "싱크대 뒤집어쓰던 날들이 끝남" 처럼 — 뭐 때문인지는 뒤에서 밝힌다. "이거 뭔데 이렇게", "이게 대체 뭐길래" 같은 지시어 오프닝은 이미 너무 흔해서 기계가 쓴 티가 나므로 쓰지 않는다.
- 상품명이나 카테고리를 초반에 드러내면 후킹이 죽는다. 본문 중반까지는 힌트만 흘리고, 정체는 후반부에 가서야 밝히거나 아예 댓글로 넘긴다.
- "괜찮다", "나쁘지 않다" 같은 미온적 반응 대신 "이걸 이제 알았다니", "왜 아무도 안 알려줬냐고" 처럼 확실한 반응으로 쓰되, 반응의 이유(무엇 때문에 그런지)는 바로 설명하지 않고 한 번 더 미룬다.
...
- [오프닝 패턴 예시 — 그대로 복사하지 말고 소재에 맞게 변형해서 쓴다] "애가 방에서 2시간째 안 나옴ㅋㅋ" / "싱크대 뒤집어쓰던 날들 이제 끝" / "남편이 사 온 건데 내가 더 씀" / "정체는 끝에 말해줌 일단 결과부터" / "왜 다들 이 얘기만 하는지 써보고 알았음"
- [마무리 패턴 예시 — 마찬가지로 변형해서 쓴다] "정체 궁금하면 댓글에서 확인", "이거 뭔지는 위에 다 나와있음", "설마 아직도 뭔지 모름?" 처럼 정체를 끝까지 안 밝히거나 마지막에 살짝 공개하는 식으로 끝맺는다. "너도 해봐/도전해봐/써봐" 계열은 아래 금지 항목이므로 이 예시로도 쓰지 않는다. "이거 뭔지 알면 바로 검색각"처럼 독자에게 직접 검색을 시키는 말투는 실제 사람의 대화체가 아니라 클릭베이트 문구처럼 들리므로 쓰지 않는다.`;
...
- "맛있다", "괜찮다" 같은 밋밋한 표현 대신 "이 정도일 줄 몰랐다", "남편이 말없이 두 그릇째 퍼감" 처럼 확실한 반응으로 쓰되, 감탄사 한 단어보다 먹는 사람의 구체적인 반응으로 보여준다.
...
- [마무리 패턴 예시 — 마찬가지로 변형해서 쓴다] "운동하는 사람 중에 아는 사람 있나", "운동하는 사람이면 진짜 알아둬야 함", "이거 모르고 운동한 거 억울함", "디테일은 댓글에 정리해둘게" 처럼 실제 대화체로 끝맺는다. "너도 해봐/도전해봐/써봐" 계열은 아래 금지 항목이므로 이 예시로도 쓰지 않는다.`;
...
- [마무리 패턴 예시 — 마찬가지로 변형해서 쓴다] "다른 엄마들은 벌써 알고 있었나", "육아템 고민 중이면 진짜 추천", "이거 모르고 산 거 너무 아까움", "자세한 건 댓글에 적어둘게" 처럼 실제 대화체로 끝맺는다. "너도 해봐/도전해봐/써봐" 계열은 아래 금지 항목이므로 이 예시로도 쓰지 않는다.`;
