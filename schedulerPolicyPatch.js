...
Module._extensions['.js'] = function patchedJsLoader(mod, filename) {
  if (!patched && path.basename(filename) === 'scheduler.js') {
    ...
    const newStart = "function startAutopilotJob(){const nextRunAt=new Map();let adminAnyRunning=false;cron.schedule('* * * * *',async()=>{";
    ...
    const newDue = "const role=String(db.prepare('SELECT role FROM users WHERE id=?').get(account.user_id)?.role||'').toLowerCase();const isAdmin=role==='admin';const due=isAdmin?0:(nextRunAt.get(account.id)||0);if(!isAdmin&&now<due)continue;if(isAdmin){if(adminAnyRunning)continue;adminAnyRunning=true;console.log(`[Autopilot][ADMIN IMMEDIATE] account #${account.id} 전역 단일실행`);}else{nextRunAt.set(account.id,now+randomIntervalMinutes()*60*1000);}try{await runAutopilotOnce(account);}";
    ...
    const newCatchEnd = "console.error(`[완전자동화 실패] account #${account.id}:`,err.response?.data||err.message);}finally{if(isAdmin)adminAnyRunning=false;}}});}";
    ...
    patched = true;
    console.log('[SCHEDULER POLICY PATCH] 관리자 즉시 자동화 전역 1건씩 + 일반회원 60~75분 유지 + ContentOnly 비활성화');
    return;
  }
