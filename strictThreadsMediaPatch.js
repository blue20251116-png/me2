...
const isThreadsCdnImage=isThreadsMediaUrl;

// Railway 저메모리 환경 보호: strict 재스크랩은 한 번에 1개만 실행한다.
...
async function scrapeStrictPostMedia(postUrl){
 return serialized(async()=>{
  ...
  try{
   ...
   // video가 lazy-load되는 경우 재생/스크롤로 network URL을 유도한다.
   ...
   const result=await page.evaluate(({sourceUrl})=>{const clean=s=>String(s||'').replace(/\s+/g,' ').trim();const canonical=href=>{try{const u=new URL(href,location.origin);return`${u.origin}${u.pathname}`.replace(/\/media$/i,'');}catch{return String(href||'').split(/[?#]/)[0].replace(/\/media$/i,'');}};const target=canonical(sourceUrl),roots=[];for(const a of document.querySelectorAll('a[href*="/post/"]')){if(canonical(a.href||'')!==target)continue;const article=a.closest('article,[role="article"]');if(article)roots.push(article);let n=a.parentElement;for(let i=0;i<10&&n;i++,n=n.parentElement){const links=[...new Set([...n.querySelectorAll('a[href*="/post/"]')].map(x=>canonical(x.href)))];if(links.length===1&&links[0]===target&&clean(n.innerText).length<=6500)roots.push(n);if(links.length>1)break;}}const root=roots.sort((a,b)=>a.getBoundingClientRect().height-b.getBoundingClientRect().height)[0]||null,images=[],videos=[];let hasVideo=false;const add=(arr,v)=>{const s=String(v||'').trim();if(/^https?:\/\//i.test(s)&&!arr.includes(s))arr.push(s);};if(root){for(const v of root.querySelectorAll('video')){hasVideo=true;add(videos,v.currentSrc);add(videos,v.src);for(const s of v.querySelectorAll('source[src]'))add(videos,s.src||s.getAttribute('src'));}for(const img of root.querySelectorAll('img')){const src=img.currentSrc||img.src||'',alt=String(img.alt||'').toLowerCase(),r=img.getBoundingClientRect();if(!src||r.width<150||r.height<150||/profile|프로필|avatar|사용자/.test(alt)||img.closest('video'))continue;add(images,src);}}const ogImage=document.querySelector('meta[property="og:image"]')?.content||'',ogVideo=document.querySelector('meta[property="og:video"],meta[property="og:video:url"],meta[property="og:video:secure_url"]')?.content||'';if(!images.length)add(images,ogImage);if(!videos.length&&ogVideo){add(videos,ogVideo);hasVideo=true;}return{foundRoot:!!root,images:images.slice(0,10),videos:videos.slice(0,5),hasVideo};},{sourceUrl:postUrl});
...
benchmark.collectPostDetails=async function strictCollectPostDetails(url,username){
 ...
 const fallbackImages=(Array.isArray(details?.images)?details.images:[]).filter(isThreadsCdnImage),fallbackVideos=(Array.isArray(details?.videos)?details.videos:[]).filter(isThreadsMediaUrl),strictImages=(strict.images||[]).filter(isThreadsCdnImage),strictVideos=(strict.videos||[]).filter(isThreadsMediaUrl);
 ...
 if(!images.length&&!videos.length&&!hasVideo)throw new Error('Threads 원본 게시물에서 사진/영상을 스크래핑하지 못했습니다');
