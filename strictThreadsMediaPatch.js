 ...
 return serialized(async()=>{
  ...
  try{
   ...
   const result=await page.evaluate(({sourceUrl})=>{
    ...
    for(const a of document.querySelectorAll('a[href*="/post/"]')){if(canonical(a.href||'')!==target)continue;addCandidate(a.closest('article,[role="article"]'));let n=a.parentElement;for(let i=0;i<12&&n;i++,n=n.parentElement)addCandidate(n);}
    ...
    const root=bestWithMedia?.node||null,images=[],videos=[];let hasVideo=false;
    ...
    if(root){for(const v of root.querySelectorAll('video')){hasVideo=true;add(videos,v.currentSrc);add(videos,v.src);for(const s of v.querySelectorAll('source[src]'))add(videos,s.src||s.getAttribute('src'));}for(const img of root.querySelectorAll('img')){const src=img.currentSrc||img.src||'',alt=String(img.alt||'').toLowerCase(),r=img.getBoundingClientRect();if(!src||badImage(src)||r.width<180||r.height<180||/profile|프로필|avatar|사용자/.test(alt)||img.closest('video'))continue;add(images,src);addSrcset(images,img.srcset);}for(const s of root.querySelectorAll('picture source[srcset],source[srcset]'))addSrcset(images,s.getAttribute('srcset'));for(const el of root.querySelectorAll('[style*="background-image"]')){const m=String(el.style.backgroundImage||'').match(/url\(["']?([^"')]+)["']?\)/i);if(m&&!badImage(m[1]))add(images,m[1]);}}
    const ogImage=document.querySelector('meta[property="og:image"]')?.content||'',ogVideo=document.querySelector('meta[property="og:video"],meta[property="og:video:url"],meta[property="og:video:secure_url"]')?.content||'';if(ogImage&&!badImage(ogImage))add(images,ogImage);if(ogVideo){add(videos,ogVideo);hasVideo=true;}
...
benchmark.collectPostDetails=async function strictCollectPostDetails(url,username){
 ...
 const strictImages=(Array.isArray(strict.images)?strict.images:[]).filter(isPostImage),strictVideos=(Array.isArray(strict.videos)?strict.videos:[]).filter(isThreadsMediaUrl);
 // 같은 post 카드에서 이미 수집된 미디어는 허용한다. 단 프로필 사진/아바타 URL은 완전 차단.
 const samePostCardImages=(Array.isArray(details?.images)?details.images:[]).filter(isPostImage),samePostCardVideos=(Array.isArray(details?.videos)?details.videos:[]).filter(isThreadsMediaUrl);
 const images=strictImages.length?strictImages:[...new Set(samePostCardImages)].slice(0,10);
 const videos=strictVideos.length?strictVideos:[...new Set(samePostCardVideos)].slice(0,5);
 if(!images.length&&!videos.length)throw new Error('선택한 Threads 원본 게시물의 미디어를 확보하지 못했습니다');
 const mode=strictImages.length||strictVideos.length?'exact-post-detail':'same-post-card-fallback';
 console.log(`[Threads][STRICT MEDIA] @${username||'-'} source=${canonicalUrl(url)} images=${images.length} videos=${videos.length} root=${strict.foundRoot?'yes':'no'} score=${strict.rootScore||0} mode=${mode} profileImageBlocked=yes externalFallback=disabled`);
 return{...details,images,videos,hasVideo:videos.length>0||!!strict.hasVideo||!!details?.hasVideo,exactUrl:true};
};

const autopilot=require('./autopilotMaterialEngine');const previousBuild=autopilot.buildThreadsFirstAutopilot.bind(autopilot);
autopilot.buildThreadsFirstAutopilot=async function strictThreadsFirstAutopilot(accountId,options){const result=await previousBuild(accountId,options);const images=(Array.isArray(result?.sourceImages)?result.sourceImages:[]).filter(isPostImage),videos=(Array.isArray(result?.sourceVideos)?result.sourceVideos:[]).filter(isThreadsMediaUrl);if(!result?.sourceUrl||(!images.length&&!videos.length))throw new Error('Threads 원본 게시물의 사진/영상을 확보하지 못해 발행하지 않습니다');result.sourceImages=images;result.sourceVideos=videos;if(result.product&&typeof result.product==='object')result.product={...result.product,image:''};console.log(`[Autopilot][STRICT THREADS MEDIA] source=${canonicalUrl(result.sourceUrl)} images=${images.length} videos=${videos.length} profileImageBlocked=yes externalFallback=disabled`);return result;};
console.log('[Threads][STRICT MEDIA PATCH V5] 정확 post 상세 우선 + 동일 post 카드 fallback 허용 · 프로필사진/타게시물/외부미디어 차단');
