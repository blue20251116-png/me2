...
Module._extensions['.js'] = function patchedJsLoader(mod, filename) {
  ...
  const delayedCode = "const hasAutoComment=!!post.auto_comment_enabled&&!!(post.link||post.recipe_comment_text);if(hasAutoComment){const commentAt=new Date(Date.now()+30*60*1000).toISOString();db.prepare(`UPDATE posts SET comment_status='pending', comment_next_retry_at=?, comment_error_message=NULL, comment_retry_count=0 WHERE id=?`).run(commentAt,post.id);console.log(`[댓글 예약] account #${account.id} post #${post.id} - 본문 발행 30분 후 ${commentAt}`);}";
  ...
  const hasThirtyMinuteSchedule = source.includes("Date.now()+30*60*1000") && source.includes("[댓글 예약]");

  if (hasPendingOnly && hasThirtyMinuteSchedule) {
    console.log('[Comment][30MIN PATCH] 본문 발행 후 30분 뒤 댓글 1회만 실행 + 자동 재시도 비활성화');
  } else {
    console.warn(`[Comment][30MIN PATCH] 최종 검증 실패 pendingOnly=${hasPendingOnly} thirtyMinute=${hasThirtyMinuteSchedule}`);
  }
