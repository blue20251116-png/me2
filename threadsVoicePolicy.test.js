...
test('runtime review repairs an overlong line instead of discarding the material', async () => {
  let calls = 0;
  const original = '이 뒤집개 하나면 프라이팬 요리가 진짜 편해짐 정말 신기함';
...
test('source is a creative seed: invented low-risk connective copy is allowed', async () => {
  ...
  const out = await policy.reviewSourceVoice(text, { sourceText: '금붕어가 먹이를 먹는 영상' }, async () => { throw new Error('low-risk creative copy should not require source-faithful semantic audit'); });
  assert.equal(out, text);
});

test('health claims still use a separate factual safety audit', async () => {
  let calls = 0;
  await assert.rejects(policy.reviewSourceVoice('이 동작이면\n허리통증이 치료됨', { sourceText: '허리 스트레칭 영상' }, async () => {calls++;return { issues: ['치료 효과 근거 없음'], sourceAnchors: [] };}),{ code: 'CONTENT_STYLE_REJECTED' });
  assert.ok(calls >= 1);
});
