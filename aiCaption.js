...
- (위에서 배정된 페르소나가 궁금증 유발형이라면) 그 페르소나는 정체를 "본문 후반부에 밝히거나" "댓글로 넘기는" 두 방식을 모두 허용하는데, 이 도구는 캡션만 만들 뿐 뒤따르는 댓글이 없다 — 그러니 반드시 "본문 후반부에 정체를 밝히는" 쪽으로만 쓰고, 위에서 금지한 "궁금하면 댓글" 같은 문구로 정체 공개를 댓글에 미루지 않는다.
