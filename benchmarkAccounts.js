...
async function openBrowser(){const playwright=require('playwright');const browser=await playwright.chromium.launch({headless:true,args:['--no-sandbox','--disable-dev-shm-usage','--disable-gpu']});const context=await browser.newContext({locale:'ko-KR',viewport:{width:1100,height:1350},userAgent:'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/140.0 Safari/537.36'});return{browser,context};}
...
async function collectProfilePostsWithContext(context,username,{limit=2}={}){
  ...
  try{
    ...
    await page.waitForTimeout(1400); await page.mouse.wheel(0,850); await page.waitForTimeout(350);
...
async function collectPostDetails(url,username){
  let browser,context;
  try{
    ({browser,context}=await openBrowser()); const page=await context.newPage(); page.setDefaultTimeout(12000);
    await page.goto(url,{waitUntil:'domcontentloaded',timeout:12000}); await page.waitForTimeout(1900); await page.mouse.wheel(0,650); await page.waitForTimeout(450);
    const data=await page.evaluate(({username,sourceUrl})=>{
      ...
      for(const a of document.querySelectorAll('a[href*="/post/"]')){
        ...
        for(let i=0;i<10&&node;i++,node=node.parentElement){
          const links=[...node.querySelectorAll('a[href*="/post/"]')].map(x=>canonical(x.href||''));
          const uniq=[...new Set(links)];
          if(uniq.length===1&&uniq[0]===targetUrl&&clean(node.innerText).length>=8)best=node;
          if(uniq.length>1&&best)break;
        }
        if(best&&!candidates.includes(best))candidates.push(best);
      }
      const main=candidates.sort((a,b)=>clean(b.innerText).length-clean(a.innerText).length)[0]||null;
      const sourceText=clean(main?.innerText||'').slice(0,3200);
      const images=[];
      if(main){for(const img of main.querySelectorAll('img')){const r=img.getBoundingClientRect();const src=img.currentSrc||img.src||'';const alt=(img.alt||'').toLowerCase();if(!src||r.width<180||r.height<180)continue;if(/profile|프로필|avatar|사용자/.test(alt))continue;if(!images.includes(src))images.push(src);}}
      const videos=[];
      if(main){for(const v of main.querySelectorAll('video')){const r=v.getBoundingClientRect();if(r.width<180||r.height<180)continue;const src=v.currentSrc||v.src||'';if(src&&!videos.includes(src))videos.push(src);}}
      const authorReplies=[];
      for(const block of document.querySelectorAll('article,[role="article"]')){
        if(block===main)continue;
        const text=clean(block.innerText);if(!text||text.length<2)continue;
        const profileLinks=[...block.querySelectorAll('a[href]')].map(a=>String(a.getAttribute('href')||'').toLowerCase());
        ...
        if(isAuthor&&!authorReplies.includes(text))authorReplies.push(text.slice(0,2200));
      }
      ...
      return{sourceText,authorReplies:authorReplies.slice(0,8),images:images.slice(0,10),videos:videos.slice(0,5),hasVideo:videos.length>0,exactUrl:canonical(location.href)===targetUrl,metaDescription};
