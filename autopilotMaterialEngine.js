...
function buildFixedProductComment(productName) {
...
공통: 원문 문장 복사 금지, 원문/작성자댓글에 없는 경험·효능·수치·재료·조리시간 창작 금지, 자연스러운 반말과 짧은 줄, 음슴체 금지, 링크·광고고지·가격 본문 금지.
레시피: 원문의 레시피 정보에 근거하고 secretTerm/쿠팡상품 정체는 본문에서 밝히지 않는다. 댓글을 봐야 핵심 재료를 알 수 있게 할 수 있다.
제품/생활: 원문의 상황과 문제를 중심으로 현재 쿠팡 상품을 자연스럽게 연결한다. '내가 본 건', '내가 산 건', '써봤는데', '사용해보니', '직접 써보니까' 등 확인되지 않은 개인 경험 표현은 절대 금지한다.
레시피인 경우에만 commentLead를 짧게 생성한다. 제품/생활은 commentLead를 빈 문자열로 출력한다.
JSON만 출력: {"text":"본문","commentLead":""}`,
  ...
  if(analysis.mode==='recipe') {
    commentLead=String(data.commentLead||'').trim() || `본문에서 말한 비밀 재료는 이거야 👇\n${productName}`;
  } else {
    // 일반상품/생활형은 AI가 댓글 문구를 바꾸지 못하게 고정한다.
    // 실제 쿠팡 고지문과 링크는 scheduler.js가 이 뒤에 자동으로 붙인다.
    commentLead=buildFixedProductComment(productName);
  }
