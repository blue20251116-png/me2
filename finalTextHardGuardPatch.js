...
engine.buildThreadsFirstAutopilot = async function finalTextHardGuardBuild(accountId, options) {
  ...
  if (reasons.length) console.log(`[AutopilotV3][TEXT HARD GUARD] v13 fix reason=${reasons.join(',')} preview="${result.text.replace(/\n/g, ' / ').slice(0,180)}"`);
...
console.log('[Autopilot][TEXT HARD GUARD] v13 기존 말투 유지 + 미완성 어미만 최소 복구');
module.exports = { hardSanitize, normalizeLineBreaks, badStyleReasons, fallbackRewrite, hasIncompleteEnding, repairIncompleteEnding };
