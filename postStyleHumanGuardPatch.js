...
// 일반상품에서 AI 후기처럼 장점을 연속으로 정리하는 패턴만 잡는다
// 단일 자연스러운 긍정 표현까지 막지 않도록 2개 이상일 때만 실패시킨다
...
function productBenefitCount(text) {
  return contentLines(text).reduce((n, line) => n + (PRODUCT_BENEFIT.test(line) ? 1 : 0), 0);
}
...
engine.buildThreadsFirstAutopilot = async function postStyleHumanGuardBuild(accountId, args = {}) {
  ...
  console.log(`[AutopilotV3][POST STYLE GUARD v15] remaining=${remaining.length?remaining.join(','):'PASS'} preview="${text.slice(0,180).replace(/\n/g,' / ')}"`);
  if (remaining.length) throw new Error(`최종 Threads 문체 검사 실패: ${remaining.join(',')}`);
  return { ...result, text };
};

console.log('[AutopilotV3][POST STYLE GUARD] v15 product 4-line cap + benefit stacking/review-template guard');
