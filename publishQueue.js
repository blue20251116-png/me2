...
for (const definition of ['publish_started_at TEXT','publish_retry_count INTEGER DEFAULT 0','publish_next_retry_at TEXT','comment_started_at TEXT','comment_creation_id TEXT']) {
    ...
    try {
      ...
      for (const post of due) {
        ...
        if (!account?.threads_access_token) {
          db.prepare("UPDATE posts SET status='failed',error_message='THREADS_TOKEN_MISSING',publish_next_retry_at=NULL WHERE id=? AND status='pending'").run(post.id);
          continue;
        }
        if (account.user_id && !canPublish(account.user_id)) {
          db.prepare("UPDATE posts SET status='failed',error_message='PUBLISH_LIMIT_OR_SUBSCRIPTION',publish_next_retry_at=NULL WHERE id=? AND status='pending'").run(post.id);
          continue;
        }
        ...
        try {
          ...
          try {
            db.prepare("UPDATE posts SET status='posted',threads_media_id=?,posted_at=?,error_message=NULL,publish_retry_count=0,publish_next_retry_at=NULL,comment_status=?,comment_next_retry_at=?,comment_retry_count=0 WHERE id=?")
          ...
          if (mediaId) {
            db.prepare("UPDATE posts SET status='posted',threads_media_id=COALESCE(?,threads_media_id),error_message='ACCOUNTING_REVIEW_REQUIRED',publish_next_retry_at=NULL WHERE id=?").run(mediaId,post.id);
          } else if (decision.retry) {
            const next = new Date(Date.now()+decision.retryDelayMs).toISOString();
            db.prepare("UPDATE posts SET status='pending',publish_retry_count=?,publish_next_retry_at=?,publish_started_at=NULL,error_message=? WHERE id=?")
              .run(decision.attempts,next,String(err.code||err.message).slice(0,500),post.id);
            console.warn(`[Publish][RETRY] account=${account.id} post=${post.id} attempts=${decision.attempts}/3 next=${next}`);
            continue;
          } else {
