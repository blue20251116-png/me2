...
const AUTO_KEYWORDS = {
  recipe: ['간단요리','집밥','자취요리','오늘뭐먹지','레시피','에어프라이어 요리','또띠아 요리','볶음밥','고기요리','야식'],
  product: ['살림템','주방용품','생활용품','자취템','청소템','수납템','차량용품','신박템','생활꿀템','운동용품'],
};
...
function shuffledKeywords(mode) {
  ...
  return page.evaluate(() => {
    ...
    for (const a of anchors) {
      ...
      for (let i = 0; i < 8 && root?.parentElement; i += 1) {
        ...
        if ((root.innerText || '').trim().length >= 25) break;
      }
      const text = String(root?.innerText || a.innerText || '').replace(/\s+/g, ' ').trim().slice(0, 900);
      const imgs = [...(root?.querySelectorAll?.('img') || [])].map(img => img.currentSrc || img.src).filter(Boolean);
      const video = root?.querySelector?.('video');
      out.push({ href, text, thumbnail: imgs[0] || '', imageCount: imgs.length, hasVideo: !!video });
    }
    return out;
  });
}
...
async function searchThreadsMaterials(keyword, { limit = 10, mode = 'recipe' } = {}) {
  ...
  const candidates = manual ? [manual] : shuffledKeywords(mode).slice(0, 4);

  let playwright;
  try { playwright = require('playwright'); }
  catch { throw new Error('Threads 검색용 Chromium이 설치되어 있지 않습니다. 최신 배포인지 확인해주세요.'); }
  ...
  try {
    ...
    for (const q of candidates) {
      ...
      for (let i = 0; i < 2; i += 1) { await page.mouse.wheel(0, 1200); await page.waitForTimeout(650); }

      const raw = await collectFromPage(page);
      const deduped = [];
      const seen = new Set();
      for (const item of raw) {
        ...
        seen.add(url);
        deduped.push({ url, text: String(item.text || '').trim(), thumbnail: String(item.thumbnail || ''), imageCount: Number(item.imageCount) || 0, hasVideo: !!item.hasVideo });
      }
      deduped.sort((a,b) => Number(b.hasVideo || b.imageCount > 0) - Number(a.hasVideo || a.imageCount > 0));
      if (deduped.length) {
        await context.close();
        return { keyword: q, attemptedKeywords: attempted, autoKeyword: !manual, searchUrl, items: deduped.slice(0, Math.max(1, Math.min(Number(limit) || 10, 20))) };
      }
    }

    await context.close();
    return { keyword: candidates[candidates.length - 1] || '', attemptedKeywords: attempted, autoKeyword: !manual, searchUrl: '', items: [] };
  } catch (err) {
