(() => {
  ...
  function dedupeAccounts(accounts){const seen=new Set(),out=[];for(const a of accounts||[]){const key=normalizeUsername(a.username);if(!key||seen.has(key))continue;seen.add(key);out.push(a);}return out;}
  ...
  function renderGroups(accounts){if(!accounts.length)return '<p style="font-size:12px;color:var(--text-dim);">아직 등록된 계정이 없습니다.</p>';return chunks(accounts,10).map((group,idx)=>{const start=idx*10+1,end=start+group.length-1;return `<details ${idx===0?'open':''} style="border:1px solid var(--border);border-radius:9px;margin-top:8px;background:var(--surface-2);overflow:hidden;"><summary style="cursor:pointer;list-style:none;padding:10px 12px;font-size:12.5px;font-weight:700;display:flex;justify-content:space-between;align-items:center;"><span>${start}~${end}번</span><span style="font-size:11.5px;color:var(--text-dim);">${group.length}개 · 펼치기/접기</span></summary><div style="padding:0 12px 6px;background:var(--surface);">${group.map(accountRow).join('')}</div></details>`;}).join('');}
  async function load(){const r=await fetch('/api/admin/benchmark-accounts'),d=await r.json();if(!r.ok){msg.textContent=d.error||'불러오기 실패';return;}const accounts=dedupeAccounts(d.accounts||[]);count.textContent=`${accounts.length}개`;list.innerHTML=renderGroups(accounts);list.querySelectorAll('[data-delete]').forEach(b=>b.onclick=async()=>{const r=await fetch('/api/admin/benchmark-accounts/'+b.dataset.delete,{method:'DELETE'});if(r.ok)load();});}
  document.getElementById('benchmarkAddBtn').onclick=async()=>{const raw=input.value.trim();if(!raw){msg.textContent='아이디를 붙여넣어주세요.';return;}const seen=new Set(),unique=[];for(const token of raw.split(/[\s,;]+/)){const v=String(token||'').trim().replace(/^@+/,'');const key=v.toLowerCase();if(!v||seen.has(key))continue;seen.add(key);unique.push(v);}if(!unique.length){msg.textContent='등록할 새 아이디가 없습니다.';return;}msg.textContent='등록 중…';const r=await fetch('/api/admin/benchmark-accounts/bulk',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({usernames:unique.join('\n')})}),d=await r.json();if(!r.ok){msg.textContent=d.error||'등록 실패';msg.style.color='var(--red)';return;}input.value='';msg.textContent=`등록 완료 · 신규 ${d.added}개 · 기존/중복 ${d.skipped}개`;msg.style.color='var(--green)';load();};
  load();
})();

(() => {
  const userList=document.getElementById('userList'); if(!userList)return;
  async function postJson(url,body){const r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:body?JSON.stringify(body):undefined});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||'처리 실패');return d;}
  async function approveForDays(userId,days){const n=Math.max(1,Math.min(3650,Math.trunc(Number(days)||0)));await postJson(`/api/admin/users/${userId}/approve`);if(n!==30)await postJson(`/api/admin/users/${userId}/grant`,{days:n-30});return n;}
  async function addDays(userId,days){const n=Math.max(1,Math.min(3650,Math.trunc(Number(days)||0)));await postJson(`/api/admin/users/${userId}/grant`,{days:n});return n;}
