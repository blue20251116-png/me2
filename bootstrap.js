  ...
  try{
    ...
    if(sourceUrl&&username){
      try{
        ...
        sourceMedia={
          images:detailImages.length?detailImages:sourceMedia.images,
          videos:detailVideos,
          hasVideo:!!details?.hasVideo||sourceMedia.hasVideo
        };
      }catch(detailErr){
        detailWarning=String(detailErr?.message||'상세 확인 실패');
        console.warn(`[material detail fallback] account #${req.account.id} @${username}: ${detailWarning}`);
      }
    }
...
console.log('[Threads material] 소재 찾기 V2 활성화');
