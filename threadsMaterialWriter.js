...
function stripAffiliateNoise(value, { preserveLines = true } = {}) {
  ...
  // 쿠팡/네이버 광고 고지 제거. 줄 중간에 붙어 있어도 제거한다.
  s = s.replace(/\[?광고\]?\s*/gi, ' ');
...
async function generateFromThreadsMaterial(accountId, { keyword, sourceText, authorReplies = '', mode = 'product' }) {
  ...
  // 중요: 원글 자체에도 쿠팡 고지/링크가 들어오는 경우가 있으므로 A/B 둘 다 먼저 정제한다.
...
- 한 문장을 길게 이어 쓰지 않고 1~2문장마다 빈 줄을 둔다.
...
A와 B에 있는 사실만 사용하고, 문장은 새로 작성할 것.`;
  ...
  const items = Array.isArray(parsed.items)
    ? parsed.items
        .map(x => ({
          text: stripAffiliateNoise(String(x?.text || ''), { preserveLines: true }),
          comment: sanitizeGeneratedComment(x?.comment)
        }))
        .filter(x => x.text)
        .slice(0, 5)
    : [];
