...
- 반응형 시작(소재를 보고 실제로 튀어나오는 감탄이나 반응)도 정상적인 선택지이며 금지하지 않는다
- 다만 특정 감탄문을 고정 템플릿처럼 반복해서 재사용하지 않는다 입력 소재에서 실제로 떠오르는 반응을 매번 새롭게 구성한다
- 최근 생성문이나 예시 문장의 첫 문장을 그대로 복사하지 않는다 동일 어휘뿐 아니라 의미상 같은 골격의 반복도 피한다
...
- 원문에 없는 구매 소유 사용 섭취 경험을 새로 만들지 않는다
- 원문에 없는 남편 아내 엄마 친구 아이 직장동료 등 관계를 새로 만들지 않는다
- 관계 주체가 원문에 명시되지 않으면 임의로 등장시키지 않는다
- 직접 경험 근거가 없으면 봤는데 보니까 영상에서 보는데 장면을 보면 같은 관찰 범위에서만 쓴다
...
console.log('[Threads][HUMAN TONE] v12 exact anchor 제거 · 사실성 규칙 명시화');
